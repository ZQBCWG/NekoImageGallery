        ...
        for file_path in self.image_files[skip:skip+top_k]:
            try:
                with Image.open(file_path) as img:
                    ...
                    # Create mapped image with all required fields
                    mapped_img = MappedImage(
                        id=str(uuid4()),  # Use UUID as required by MappedImage model
                        index_date=datetime.now(),  # Set current time as index date
                        url=f"http://localhost:8000/static/{os.path.basename(file_path)}",  # Use absolute URL with original filename
                        thumbnail_url=None,  # Thumbnail will be generated by storage service
                        ocr_text=None,
                        ...
                        starred=False,
                        categories=[],
                        local=True,  # Mark as local image
